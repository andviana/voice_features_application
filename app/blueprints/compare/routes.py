...
# Usar helpers de min/max/flat do próprio routes para montar as escalas
...
def compare_view():
    ...
    patients = {g: [] for g in sorted(PathUtils.ALLOWED_GROUPS_LIST)}

    for group in sorted(PathUtils.ALLOWED_GROUPS_LIST):
        try:
...
def compare_info(group: str, filename: str):
    ...
    espalhando o mesmo JSON robusto da rota scientific_analysis.
    """
    ...
    # Processamento de Sinais para Gráficos
    ...
    payload = {
        ...
        "raw": {
            "play_url": f"/compare/play/raw/{group}/{filename}",
            "props": raw_props,
            "waveform": wave_raw,
            "spectrum": spec_raw,
        ...
        "processed": {
            "play_url": f"/compare/play/processed/{group}/{filename}",
            "props": proc_props,
            "waveform": wave_proc,
            "spectrum": spec_proc,
